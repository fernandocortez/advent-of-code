...
let sum = 0;
let paperRollsMovedThisRound = 0;
do {
	paperRollsMovedThisRound = 0;
		...
		row.forEach((char, colIdx) => {
			if (char === "@") {
				...
				if (adjacentSubGrid.filter((char) => char === "@").length < 4) {
					paperRollsMovedThisRound++;
					data[rowIdx][colIdx] = ".";
				}
			}
		});
	});
	sum += paperRollsMovedThisRound;
} while (paperRollsMovedThisRound > 0);
console.log("Paper rolls moved:", sum);
